...
meter_param=(
		...
		'--worker 1 --loop 1 --pps 20000 eth5 /home/meter/object_summary1.pcapng'
...
And_meter_param=(
		'--worker 1 --loop 20 --pps 25000 eth5 /home/meter/sto2.pcap'
		'--worker 1 --loop 20 --pps 20000 eth5 /home/meter/sto2.pcap'
)
			...
			if [[ meter_flag -gt 0 ]];then
				if [[ $num -eq 30 ]];then
					...
					for i in `seq 1 ${#And_meter_param[@]}`
						do
							And_meter${i}_param_tmp=`echo ${$((2+i))} |sed 's/-/\\\-/g'`

							And_meter${i}_flag=`ssh root@$meter_ip ps -ef |grep "$And_meter${i}_param_tmp" |grep -v "grep"|wc -l`

							if [[ $And_meter -eq 1 ]] && [[ $And_meter${i}_flag -eq 0 ]];then
								ssh root@$meter_ip "nohup ${dbfw_home}/sbin/meter_broadcast ${$((2+i))} >/dev/null 2>&1 &"
								((And_meter_num=And_meter_num+1))
								printf_log 1 "And_meter${i} Begin run!,And_meter${i}_param: ${$((2+i))}"
							else
								printf_log 1 "And_meter${i} running!"
							fi
					...
					if [[ $And_meter -eq 1 ]] && [[ $And_meter_flag -eq 0 ]];then
								ssh root@$meter_ip "nohup ${dbfw_home}/sbin/meter_broadcast $3 >/dev/null 2>&1 &"
						printf_log 1 "And_meter Begin run!,And_meter_param: $3"
						((And_meter_num=And_meter_num+1))
					fi
...
for meter_param_Num in "${meter_param[@]}"  
	do
		...
		meter_RunAndMon "$meter_param_Num" "$meter_Num" "${And_meter_param[@]}"
		##已运行完的meter打包耗时
...
#				((And_meter_num=And_meter_num+1))
#				printf_log 1 "And_meter run num=$And_meter_num"	
...
while true
	do
		...
		if [[ $tlog_count -eq 0 ]];then
			...
			##kill 并行meter
			And_meter_flag=`ssh root@$meter_ip ps -ef|grep "$And_meter_param_tmp" |grep -v "grep"|wc -l`
			if [[ $And_meter -eq 1 ]] && [[ $And_meter_flag -eq 1 ]];then
			...
			if [[ $And_meter -eq 1 ]] && [[ $And_meter_flag -eq 0 ]];then
				ssh root@$meter_ip "nohup ${dbfw_home}/sbin/meter_broadcast $3 >/dev/null 2>&1 &"
				printf_log 1 "And_meter Begin run!, And_meter_param: $And_meter_param"
				((And_meter_num=And_meter_num+1))
			fi
			sleep 1
			continue
